...
export async function create(req, res) {
    ...
    const { task, completed, belongsToID } = req.body;
    console.log(`${task} task received and is completed? ${completed}`);
    if (task === undefined || completed === undefined || belongsToID === undefined) {
      return res.status(400).send("Missing todo");
    }
    const todo = await Todo.createTodo({ task, completed, belongsToID });
    console.log({ todo });
...
export async function update(req, res) {
  ...
  const todo = await Todo.updateTodo(id, { task, completed, belongsToID });
  console.log({ todo });
